...
abelescalcall(void *userdata,
	...
	*/
	for(ii = 1; ii < nlayers + 1;ii += 1){
        ...
        sld of the layer
        */
		numtemp = 1.e-6 * ((100. - coefP[4 * ii + 4]) / 100.) * coefP[4 * ii +3]
                    + (coefP[4 * ii + 4] * coefP[3] * 1.e-6) / 100.;
	...
	if(Vmullayers > 0 && Vmulrep > 0 && Vmulappend >= 0){
		...
		*/
		...
		for(ii=0; ii<Vmullayers;ii+=1){
			numtemp = (coefP[3] * 1e-6 * coefP[(4 * ii) + offset + 2] / 100)
			 + (1e-6 * ((100 - coefP[(4 * ii) + offset + 2]) / 100) * coefP[(4 * ii) + offset + 1]);		//sld of the layer
	...
	for (j = 0; j < npoints; j++) {
        ...
        */
		for(ii=0; ii<nlayers+2 ; ii++){
			pj[ii] = (*(SLDmatrix + ii) > qq) ?
                    compsqrt(qq2 - MyComplex(*(SLDmatrix + ii), 0)) :
                    MyComplex(sqrt(qq - *(SLDmatrix + ii)), 0);
		}

		/*
		workout the wavevector in the toplayer of the multilayer, if it
		exists.
		*/
		if(Vmullayers > 0 && Vmulrep > 0 && Vmulappend >= 0){
			memset(subtotal, 0, sizeof(subtotal));
			subtotal[0][0] = MyComplex(1, 0);
			subtotal[1][1] = MyComplex(1, 0);
			pj_mul[0] = (*(SLDmatrixREP) > qq) ?
                     compsqrt(qq2-MyComplex(*SLDmatrixREP, 0)) :
                     MyComplex(sqrt(qq-*SLDmatrixREP),0);
		}

		//now calculate reflectivities
		for(ii = 0 ; ii < nlayers + 1 ; ii++){
			/*
			work out the fresnel coefficients. This looks more complicated than
			it really is. The reason it looks so convoluted is because if there
			is no complex part of the wavevector, then it is faster to do the
			calc with real arithmetic then put it into a complex number.
			*/
			if(Vmullayers > 0 && ii == Vmulappend && Vmulrep > 0 ){
				...
				if((pj[ii]).im == 0 && (pj[ii+1]).im == 0){
                    ...
                    rj.re = (ii == nlayers) ?
                         ...
                         * exp(anum * anum2 * -2 * subrough * subrough)):
                        (((anum - anum2) / (anum + anum2))
                         * exp(anum * anum2 * -2 * coefP[4 * (ii + 1) + 5]
                         * coefP[4 * (ii + 1) + 5]));

					rj.im = 0;
				} else {
					rj = (ii == nlayers) ?
                        (((pj[ii] - pj[ii + 1]) / (pj[ii] + pj[ii + 1]))
                         * compexp(pj[ii] * pj[ii + 1]
                                   * MyComplex(-2 * subrough * subrough, 0))):
                        (((pj[ii] - pj[ii + 1]) / (pj[ii] + pj[ii + 1]))
                         * compexp(pj[ii] * pj[ii + 1]
                                   * MyComplex(-2 * coefP[4 * (ii + 1) + 5]
                                               * coefP[4 * (ii + 1) + 5], 0)));
				};
			}
			...
			if(Vmullayers > 0 && ii == Vmulappend && Vmulrep > 0){
				...
				for(kk = 0; kk < Vmulrep; kk++){
				//if you are in the last multilayer
					if(kk == Vmulrep - 1){
						for(jj = 0; jj < Vmullayers; jj++){
							beta = compexp((MyComplex(0,fabs(coefP[4*jj+offset]))*pj_mul[jj]));
...
//	extern int NUM_CPUS;
	...
	err = abelescalcall(userdata,
	                    coefs,
	                    model + pointsConsumed,
	                    (*xdata) + pointsConsumed,
	                    pointsRemaining,
	                    0,
	                    0,
	                    0);
...
int abelesmodelwrapper(void *userdata,
                       const double *coefs,
                       unsigned int numcoefs,
                       double *model,
                       const double **xdata,
